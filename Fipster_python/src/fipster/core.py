...
class FIP_signal:
    ...
    def load_data_matlab(self, filename):
        ...
        with h5py.File(filename, mode='r') as data:
            ...
            for key, value in data.items():
                ...
                if key == 'framerate':
                    # Verify framerate is a single number
                    try:
                        self.framerate = int(value[()])
                    except:
                        self.framerate = int(value[0][0])
    ...
    def check_settings(self):
        ...
        if invalid_keys:
            ...
            raise Exception("{0} is not a valid setting... removed".format(', '.join(invalid_keys)))
    ...
    def get_data(self):
        ...
        if self.detrend:
            window = self._resolve_savgol_window(data.shape[1], preferred=5001)
            if window is not None:
                data[0, :, :] = (
                    data[0, :, :]
                    - scipy_signal.savgol_filter(data[0, :, :], window, 1, axis=0)
                    + np.min(data[0, :, :], axis=0)
                )

        # Should we smooth?
        for i in range(0, self.nr_signals):
            if self.smooth[i]:
                window = self._resolve_savgol_window(data.shape[1], preferred=self.smooth[i])
                if window is not None:
                    data[0, :, i] = scipy_signal.savgol_filter(data[0, :, i], window, 1)
    ...
    def get_ref(self, detrend=False):
        ...
        if detrend:
            window = self._resolve_savgol_window(ref.shape[1], preferred=5001)
            if window is not None:
                for i in range(0, self.nr_signals):
                    ref[0, :, i] = ref[0, :, i] - \
    ...
    def derive_timestamps(self, TTL = 'all', names = 'default', min_length = 0,
        ...
        for i in range(0, len(columns)):
            ...
            if store_stamps:
                self.timestamps[name+'_onset'] = stamps['start']
                self.timestamps[name+'_offset'] = stamps['end']
        ...
        for ttl in TTL:
            if ttl in self.timestamps.keys():
                ...
                stamps = self.derive_timestamps(ttl, min_length=stamps_min, max_length=stamps_max,
                        store_stamps = False)
                if isinstance(stamps, pd.DataFrame):
                    stamps = {str(ttl): stamps}
    ...
    def find_signal_switch(self):
        ...
        first_signal = raw_signal[0, 0, :][np.newaxis, :]
        first_ref = raw_ref[0, 0, :][np.newaxis, :]
        d_signal = np.diff(raw_signal[0, :, :], axis=0, prepend=first_signal)**2
        d_ref = np.diff(raw_ref[0, :, :], axis=0, prepend=first_ref)**2
    ...
    def _resolve_savgol_window(self, data_length, preferred):
        """
        Return a valid Savitzky-Golay window length for a given signal length.
        Returns None when the signal is too short to filter safely.
        """
        window = int(preferred)
        if window < 3 or data_length < 3:
            return None

        window = min(window, data_length)
        if window % 2 == 0:
            window -= 1
